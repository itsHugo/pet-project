...
describe('User Tests', () => {
    describe('#login()', () => {
        it('should throw error when try to login with wrong pass or username', (done) => request.post(url + '/auth/login', { json: { Email: 'dmitrevski2@gmail.com', Password: '123456' } }, (err, res, body) => {
            expect(res.statusCode == 400);
            expect(body.status == 'error');
            done(err);
        }));
        it('should not throw error when try to login with valid parameters', (done) => request.post(baseUrl + '/login', { json: { Email: 'dmitrevski2@gmail.com', Password: '12345' } }, (err, res, body) => {
